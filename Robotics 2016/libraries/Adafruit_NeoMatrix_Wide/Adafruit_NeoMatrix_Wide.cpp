/*-------------------------------------------------------------------------
  ...
  the Adafruit_GFX_Wide graphics library.  Requires both the Adafruit_NeoPixel
  and Adafruit_GFX_Wide libraries.
...
#include <Adafruit_NeoMatrix_Wide.h>
#include "gamma_wide.h"
#ifdef __AVR__
...
Adafruit_NeoMatrix_Wide::Adafruit_NeoMatrix_Wide(int w, int h, uint8_t pin,
  uint8_t matrixType, uint8_t ledType) : Adafruit_GFX_Wide(w, h),
  Adafruit_NeoPixel(w * h, pin, ledType), type(matrixType), matrixWidth(w),
  matrixHeight(h), tilesX(0), tilesY(0), remapFn(NULL) { }

// Constructor for tiled matrices:
Adafruit_NeoMatrix_Wide::Adafruit_NeoMatrix_Wide(uint8_t mW, uint8_t mH, uint8_t tX,
  uint8_t tY, uint8_t pin, uint8_t matrixType, uint8_t ledType) :
  Adafruit_GFX_Wide(mW * tX, mH * tY), Adafruit_NeoPixel(mW * mH * tX * tY, pin,
  ledType), type(matrixType), matrixWidth(mW), matrixHeight(mH), tilesX(tX),
  tilesY(tY), remapFn(NULL) { }

// Expand 16-bit input color (Adafruit_GFX_Wide colorspace) to 24-bit (NeoPixel)
// (w/gamma adjustment)
...
uint16_t Adafruit_NeoMatrix_Wide::Color(uint8_t r, uint8_t g, uint8_t b) {
...
// pixel(s), side-stepping the 16-bit color limitation of Adafruit_GFX_Wide.
...
void Adafruit_NeoMatrix_Wide::setPassThruColor(uint32_t c) {
  passThruColor = c;
  passThruFlag  = true;
}

// Call without a value to reset (disable passthrough)
void Adafruit_NeoMatrix_Wide::setPassThruColor(void) {
  passThruFlag = false;
}

void Adafruit_NeoMatrix_Wide::drawPixel(int16_t x, int16_t y, uint16_t color) {
...
void Adafruit_NeoMatrix_Wide::fillScreen(uint16_t color) {
  uint16_t i, n;
  uint32_t c;
...
void Adafruit_NeoMatrix_Wide::setRemapFunction(uint16_t (*fn)(uint16_t, uint16_t)) {
  remapFn = fn;
}
